...
No `variants` — the `quadlet/` directory is built as a single artifact. The `+quadlet` build metadata suffix
prevents tag collision with the margo artifact (which uses the root `version` as its tag).

!!! warning
    The `+` character is not valid in OCI tags. margot automatically converts `+` to `_` when pushing, so
    `1.0.0+quadlet` becomes OCI tag `1.0.0_quadlet`. Write versions with `+` in `margo.yaml` — the conversion is
