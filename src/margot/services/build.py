...
def build(
    ...
    *,
    ...
    if package_type == PackageType.ALL:
        ...
        targets.extend(
            _build_compose_or_quadlet(
                meta, project_dir, build_dir, version_override, variant,
                PackageType.COMPOSE, placeholders
            )
        )
...
def _build_placeholder_map(meta: MargoYaml, version_override: str | None) -> dict[str, str]:
    ...
    compose_version = (
        version_override
        or (meta.compose.version or (meta.compose.variants[0].version if meta.compose.variants else "")) if meta.compose else "")
    quadlet_version = (
        version_override
        or (meta.quadlet.version or (meta.quadlet.variants[0].version if meta.quadlet.variants else "")) if meta.quadlet else "")
...
def _build_compose_or_quadlet(  # noqa: PLR0913
    ...
    if not component.variants:
        ...
        return _build_flat_component(
            meta, component, project_dir, build_dir, version_override,
            variant, component_type, placeholders
        )
...
def _build_variant_component(  # noqa: PLR0913
    ...
    for v in variants_to_build:
        ...
        validate_semver(version)
        console.info(
            f"Building {component_name} variant '{v.name}': version {version}"
        )
        ...
        try:
            ...
            make_tarball(tmp_dir, output_path)
            console.info(
                f"{component_name} variant '{v.name}' built: {output_path}"
            )
        finally:
            shutil.rmtree(tmp_parent, ignore_errors=True)
